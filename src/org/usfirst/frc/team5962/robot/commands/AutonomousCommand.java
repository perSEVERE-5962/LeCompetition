...
//default//start(x); makes the robot start again after being told to stop()
		 //delay(x, y); tells the program to wait x seconds before reading the next line
//default//forward(x); makes the robot go forward
		 ...
		 //x = times specific command was called, start with 1 and count up for times called. (ex, forward(1) then forward(2))
